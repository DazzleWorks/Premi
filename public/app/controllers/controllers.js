'use strict';

angular.module('app.controllers',
    [
        'ngRoute',
        //'angularFileUpload',
        'app.controllers.DeleteInfographicCtrl',
        'app.controllers.DeleteProjectCtrl',
        'app.controllers.ForgotPasswordCtrl',
        'app.controllers.GenericProjectCtrl',
        'app.controllers.HomePageCtrl',
        'app.controllers.ImageCtrl',
        'app.controllers.InfographicEditorCtrl',
        'app.controllers.LoginCtrl',
        'app.controllers.MyAccountCtrl',
        'app.controllers.MyProjectsCtrl',
        'app.controllers.NewProjectCtrl',
        'app.controllers.PresentationCtrl',
        'app.controllers.PresentationEditorCtrl',
        'app.controllers.PresentationStyleCtrl',
        'app.controllers.RenameProjectCtrl',
        'app.controllers.ResetPasswordCtrl',
        'app.controllers.RestrictedAreaCtrl',
        'app.controllers.SignupCtrl',
        'app.controllers.SlideEditorCtrl',
        'app.controllers.TextCtrl'
    ]
);
